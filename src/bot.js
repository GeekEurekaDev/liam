...
client.on(Discord.Events.ClientReady, async () => {
    setInterval(async () => {
        // Fetch all mute data from the database
        const allMuteData = await MutedSchema.find({});

        allMuteData.forEach(async (muteData) => {
            // If the mute time has passed
            if (new Date() > muteData.expires) {
                ...
                // If the guild or the user does not exist, return
                if (!guild || !member) return;

                // Fetch the 'muted' role
                const mutedRole = guild.roles.cache.find(role => role.name === "Muted");

                // If the 'muted' role does not exist, return
                if (!mutedRole) return;

                // Remove the 'muted' role from the user
                await member.roles.remove(mutedRole);

                // Delete the mute data from the database
                await MutedSchema.deleteOne({ guildId: muteData.guildId, userId: muteData.userId });
            }
        });
